...
function createBookingsList(start, end, events, timestep) {
  ...
  return new Promise(resolve => {
    while (currentDate < end) {
      bookingsList.push({
        time: new Date(currentDate),
        availableToBook: Math.random() > 0.3
      });
...
const endpoints = {
  addPaymentToBooking: (id, paymentAmount) => {
    return api.addPaymentToBooking(id, paymentAmount);
  },
  ...
  getBookings: (start, end) => {
    return api.getBookings(start, end).then(json => {
      return createBookingsList(start, end, json, 30);
    });
  },

  getBookings2: (start, end, isWalkins) => {
    return api.getBookings2(start, end, isWalkins).then(bookings => {
